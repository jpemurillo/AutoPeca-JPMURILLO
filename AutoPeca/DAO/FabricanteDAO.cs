   ...
   public class FabricanteDAO : BaseDAO
    {
        ...
        public void incluir()
        {
            try
            {
                string sql = "insert into tb_fabricante (NM_NOME,NM_DESC) " +
                    "values (@nom,@desc)";
                db.AddParameter("@nom", vo.nome, ParameterDirection.Input);
        ...
        {
            string sql = $"SELECT id,nm_nome,nm_desc from tb_fabricante where id=@id";
            db.AddParameter("@id", id, ParameterDirection.Input);
            try
            {
